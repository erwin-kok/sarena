...
use dataplane_common_test::{ScapyAssert, TEST_RESULT_MAP_SIZE, TestStatus, tlv_reader};
use regex::Regex;
...
fn ebpf_test_runner() -> Res<()> {
    ...
    let dir = std::env::var("EBPF_DIR").unwrap_or_else(|_| "/usr/lib/dataplane/ebpf".into());

    let mut prod_bpf = Ebpf::load_file(format!("{dir}/dataplane-ebpf-programs.o"))?;
    let mut test_bpf = Ebpf::load_file(format!("{dir}/dataplane-ebpf-test-programs.o"))?;
