...
use sarena_infra::{
    InfraError, InterfaceAddress, Link, NetlinkNetworkProvisioner, Netns, NetworkProvisioner,
    VethSpec, test_support,
};
...
fn v4(ip: Ipv4Addr, prefix: u8) -> InterfaceAddress {
    InterfaceAddress::new(IpAddr::V4(ip), prefix).expect("valid IPv4 network")
}
